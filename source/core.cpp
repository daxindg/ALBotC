...
std::unordered_map<std::string, std::unordered_map<std::string, std::function<void(Pos)>>> processer{
    {"at", {
        {"main", [](Pos pos){
            if (enableCombat) {
        ...
        {"combat", [](Pos withdraw){
            ...
            if (state.enemy.empty()) {
                ...
                state.enemy.resize(std::partition(state.enemy.begin(), state.enemy.end(), [](Pos p){return !(p.first < 845 && p.second < 240) && p.second > 100 && !(p.first >= 970 && p.second >= 945) && !(p.second < 814 && p.second > 583 && p.first > 1740);}) - state.enemy.begin());
            }
